 ...
 */
package org.openmrs.module.pihlesotho.web.dwr;
...
public class RegimenPickupDWRService {
     ...
     */
         ...
         try {
	         ...
	         if (obsToDelete != null && obsToDelete.getPersonId().equals(patientId)){
		         ...
		         if (encToDelete == null) {
					 ...
					 EncounterSearchCriteriaBuilder cb = new EncounterSearchCriteriaBuilder();
					 cb.setPatient(patient);
					 cb.setFromDate(obsToDelete.getObsDatetime());
					 cb.setToDate(obsToDelete.getObsDatetime());
					 cb.setEncounterTypes(ets);
					 cb.setIncludeVoided(false);
		        	 List<Encounter> encs = es.getEncounters(cb.createEncounterSearchCriteria());
     ...
     public boolean addPickup(String regimen, String dateString, String locationId, Integer patientId) {
    	 try {
			 ...
			 e.addProvider(getDefaultProviderRole(), getDefaultProvider());
	 ...
	 * Utility method to retrieve the default Provider by id
	 */
	private EncounterRole getDefaultProviderRole() {
		EncounterRole p = null;
		try {
			String roleLookup = Context.getAdministrationService().getGlobalProperty("regimenpickup.defaultProviderRole");
			p = Context.getEncounterService().getEncounterRoleByName(roleLookup);
			if (p == null) {
				p = Context.getEncounterService().getEncounterRoleByUuid(roleLookup);
			}
		}
		catch (Exception e) {
		}
		if (p == null) {
			throw new RuntimeException("Unable to find default provider role.  Please configure your global properties.");
		}
		return p;
	}
}
