...
public class CoreConceptMetadataBundle extends VersionedMetadataBundle {
	...
	protected void installNewVersion() throws Exception {
		...
		install(conceptClass("Frequency",
		    "A concept used for capturing frequency information such as for medication ordering.", ConceptClasses.FREQUENCY));
		
		install(conceptSource("PIH", "Partners In Health concept dictionary using concept ids and preferred English names",
		    null, ConceptSources.PIH));
		...
		install(conceptSource("SNOMED MVP", "MVP Namespace Identifier extensions to SNOMED CT", null,
		    ConceptSources.SNOMED_MVP));
		install(conceptSource("RxNORM", "RxNORM CUI", null, ConceptSources.RX_NORM));
		install(conceptSource("IMO ProcedureIT", "Intelligent Medical Objects, Inc. procedure/test datasource", null,
		    ConceptSources.IMO_PROCEDURE_IT));
		install(conceptSource("IMO ProblemIT", "Intelligent Medical Objects, Inc. problem/disease/finding datasource", null,
		    ConceptSources.IMO_PROBLEM_IT));
		...
		install(conceptSource("LOINC", "A universal code system for identifying laboratory and clinical observations.",
		    null, ConceptSources.LOINC));
		install(conceptSource("org.openmrs.module.emrapi", "Source used to tag concepts used in the emr-api module", null,
		    ConceptSources.EMRAPI_MODULE));
		install(conceptSource("org.openmrs.module.mdrtb", "Concepts used with the MDR-TB module", null,
		    ConceptSources.MDRTB_MODULE));
		install(conceptSource("org.openmrs.module.mirebalaisreports", "Concepts used for HUM reports", null,
		    ConceptSources.HUM_REPORTS_MODULE));
	}
